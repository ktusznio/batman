...
class Binding extends Batman.Object
  ...
  constructor: ->
    ...
    # And attach them.
    if @only != 'target' and Batman.DOM.nodeIsEditable(@node)
...
class RenderContext
  ...
  # The optional `only` parameter can be used to create source-only or target-only bindings. If left unset,
...
Batman.DOM = {
  ...
  readers: {
    ...
    target: (node, key, context, renderer) ->
      Batman.DOM.readers.bind(node, key, context, renderer, 'target')
  ...
  attrReaders: {
    ...
    target: (node, attr, key, context) ->
      Batman.DOM.attrReaders.bind node, attr, key, context, 'target'
