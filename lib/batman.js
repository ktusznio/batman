(function() {
  ...
  Binding = (function() {
    ...
    function Binding() {
      ...
      shouldSet = true;
      if (this.only !== 'target' && Batman.DOM.nodeIsEditable(this.node)) {
  ...
  Batman.DOM = {
    readers: {
      ...
      target: function(node, key, context, renderer) {
        return Batman.DOM.readers.bind(node, key, context, renderer, 'target');
      },
    ...
    attrReaders: {
      ...
      target: function(node, attr, key, context) {
        return Batman.DOM.attrReaders.bind(node, attr, key, context, 'target');
      },
